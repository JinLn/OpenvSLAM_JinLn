...
void stereo_tracking(const std::shared_ptr<openvslam::config>& cfg,
    ...
    const std::string left_image_file_path=sequence_dir_path+"/image_0/";
    const std::string right_image_file_path=sequence_dir_path+"/image_1/";                   
    const image_sequence left_sequence(left_image_file_path, cfg->camera_->fps_);
    const image_sequence right_sequence(right_image_file_path, cfg->camera_->fps_);

    //const kitti_sequence sequence(sequence_dir_path);
    const auto left_frames = left_sequence.get_frames();
    const auto right_frames = right_sequence.get_frames();
    // build a SLAM system
    ...
    track_times.reserve(left_frames.size());

    // run the SLAM in another thread
    std::thread thread([&]() {
        for (unsigned int i = 0; i < left_frames.size(); ++i) {
            const auto& left_frame = left_frames.at(i);
            const auto& right_frame = right_frames.at(i);
            ...
            if (!left_img.empty() && !right_img.empty() && (i % frame_skip == 0)) {
                ...
                SLAM.feed_stereo_frame(left_img, right_img, left_frame.timestamp_);
            }
            ...
            if (!no_sleep && i < left_frames.size() - 1) {
                const auto wait_time = left_frames.at(i + 1).timestamp_ - (left_frame.timestamp_ + track_time);
                if (0.0 < wait_time) {
